...
ls -1 original/*_v1.vcf.gz |cut -f 2 -d'/' |while read i
    do
        ...
        v7=$(echo $v3 |sed 's/p12/p12_f2/g')
        v8=$(echo $v3| sed 's/p12/p12_filterNew_onTarget/g'| sed 's/vcf/vcf\.gz/g')
        sbatch script/pipeline_populated/pre_3c.sh $i $v2 $v3 $v4 $v5 $v6 $v7 $v8
    done
