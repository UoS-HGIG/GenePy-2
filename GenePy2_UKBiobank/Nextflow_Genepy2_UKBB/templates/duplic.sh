...
OUTPUT_FOLDER="$output_path/concatenated_genes_${chromosome_name}"
...
echo "$folder"/metafiles >> "$FINAL_LIST"
    for file in "$folder"/metafiles/*.meta; do
...
if [ -d "$OUTPUT_FOLDER" ]; then
  echo "${output_path}/concatenated_genes_${chromosome_name}" >> "$FINAL_LIST"
fi
